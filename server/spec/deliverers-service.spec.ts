...
describe('O servico de entregadores', () => {
  ...
  it('Cadastro De Usuario Realizado', () => {
    const addOne = deliverersService.addDeliverer(deliverer1)
    const addTwo = deliverersService.addDeliverer(deliverer2)
  ...
  it('Cadastro De Usuario Nao Realizado CNH ja cadastrada', () => {
    const addOne = deliverersService.addDeliverer(deliverer1)
    const addTwo = deliverersService.addDeliverer(deliverer1)
  ...
  it('Cadastro De Usuario Nao Realizado Dado Ausente no Cadastro)', () => {
    const deliverer = new Deliverer('João da Silva',
      'js@email.com',
      '1234',
      '',
      '25769318041',
    ...
    const add = deliverersService.addDeliverer(deliverer)
  ...
  it('Valida Email e Senha do Usuario com Sucesso', () => {
    ...
    const correctEmail = 'js@email.com'
    const correctPassword = '1234'
    const delivererLogged = deliverersService.validateCredentials(correctEmail, correctPassword)
  ...
  it('Nao Valida Email e Senha do Entregador Email Incorreto', () => {
    deliverersService.addDeliverer(deliverer1)
    const wrongEmail = 'wrongEmail@email.com'
    const delivererLogged = deliverersService.validateCredentials(wrongEmail, deliverer2.Password)
  ...
  it('Nao Valida Email e Senha do Entregador Senha Incorreta', () => {
    ...
    const delivererLogged = deliverersService.validateCredentials(deliverer1.Email, wrongPassword)
  ...
  it('Nao Valida Email e Senha do Entregador Dado Nao Preenchido', () => {
    deliverersService.addDeliverer(deliverer1)
    const delivererLogged = deliverersService.validateCredentials('', deliverer2.Password)
  ...
  it('Encontra Entregador a Partir do ID com Sucesso', () => {
    deliverersService.addDeliverer(deliverer1)
    const correctID = deliverersService.Deliverers[0].ID
    const deliverer = deliverersService.getById(correctID)
  ...
  it('Nao Encontra Entregador a Partir do ID ID Incorreta ou Ausente', () => {
    ...
    const deliverer = deliverersService.getById(deliverer2.ID)
  ...
  it('Encontra Entregadores Disponiveis para Entrega com Sucesso', () => {
    ...
    const availables = deliverersService.getAvailables()
